...
export type EmpFilterKey = 'empCode' | 'empName' | 'empEmailId' | 'empPhoneno';
...
export class EmployeetableComponent implements OnInit, AfterViewInit {
  displayedColumns: string[] = ['empCode', 'empName', 'empPhoneno', 'empEmailId', 'action'];
  ...
  sortedColumns: { [key in EmpFilterKey]?: 'asc' | 'desc' | '' } = {
    empCode: '',
    empName: '',
    empEmailId: '',
    empPhoneno: '',
  ...
  @ViewChild('commonFilterBox') commonFilterBox!: ElementRef;
  ...
  ngOnInit(): void {
    this.dataSource.filterPredicate = (data: Employee, filter: string): boolean => {
      ...
      const columnMatch =
        (!searchItems.empCode ||
          (data.empCode ?? '').toLowerCase().includes(searchItems.empCode.toLowerCase())) &&
        (!searchItems.empName ||
          (data.empName ?? '').toLowerCase().includes(searchItems.empName.toLowerCase())) &&
        (!searchItems.empEmailId ||
          (data.empEmailId ?? '').toLowerCase().includes(searchItems.empEmailId.toLowerCase())) &&
        (!searchItems.empPhoneno ||
          (data.empPhoneno ?? '').toLowerCase().includes(searchItems.empPhoneno.toLowerCase()));

      const globalMatch =
        !searchItems.global ||
        (data.empCode ?? '').toLowerCase().includes(searchItems.global) ||
        (data.empName ?? '').toLowerCase().includes(searchItems.global) ||
        (data.empEmailId ?? '').toLowerCase().includes(searchItems.global) ||
        (data.empPhoneno ?? '').toLowerCase().includes(searchItems.global);

      const isMatch = globalMatch && columnMatch;

      if (isMatch) {
        console.log('Matched row :', data, 'Filter:', searchItems);
      }
      return isMatch;
    };
    ...
    this.getEmployees();
    // set the empCode
    this.loggedInEmpCode = localStorage.getItem('EmpCode') || '';
  ...
  ngAfterViewInit(): void {
    ...
    this.sort.sortChange.subscribe((state) => {
      ...
      if (state.direction && state.active) {
        this.sortedColumns[state.active as EmpFilterKey] = state.direction;
      }
  ...
  editEmployee(empCode: string): void {
    ...
    const emp = this.dataSource.data.find((e) => e.empCode === empCode);
  ...
  deleteEmployee(empCode: string): void {
    ...
    if (empCode.trim().toLowerCase() === this.loggedInEmpCode.trim().toLowerCase()) {
      this.toast.showError('You Cannot delete your own account while logged in');
      return;
    }

    this.toast.showConfirm('Are you sure you want to delete this employee?', () => {
      this.employeeService.deleteEmployee(empCode).subscribe({
        next: (res) => {
          this.dataSource.data = this.dataSource.data.filter((emp) => emp.empCode !== empCode);
          this.refreshPaginator();
        ...
        error: (err) => {
          this.toast.showError('Employee Delete Failed.Try again');
  ...
  resetAll() {
    ...
    console.log('Filtered data before sort:', this.dataSource.filteredData);
    // Clear MatSort UI state
    this.sort.active = '';
    this.sort.direction = '';
    this.sort.sortChange.emit({ active: '', direction: '' }); // IMPORTANT
    this.dataSource.sort = this.sort;
  }
  ...
  saveFilterState() {
    ...
    this.employeeService.saveFilterState(payload).subscribe({
      next: () => this.toast.showSuccess('Filter & Sort saved'),
      error: () => this.toast.showError('Failed to save Filter & Sort'),
    });
  }
  ...
  applyFilter(event: Event) {
    if (!this.filterValues) {
      this.filterValues = { empCode: '', empName: '', empEmailId: '', empPhoneno: '', global: '' };
    }
  ...
  openColumnFilter(column: EmpFilterKey, event?: MouseEvent) {
    ...
    if (event) {
      ...
      if (thElement) {
        ...
        this.filterBoxTop = rect.bottom + window.scrollY + 5;
    ...
    setTimeout(() => {
      const inputE1 = this.filterInputs?.find(
        (e1) => e1.nativeElement.getAttribute('data-column') === column,
      )?.nativeElement;
  ...
  resetColumnFilter(column: EmpFilterKey) {
    ...
    const input = this.filterInputs.find(
      (el) => el.nativeElement.getAttribute('data-column') === column,
    );
    if (input) {
      input.nativeElement.value = '';
    }
  ...
  loadSavedFilter() {
    this.employeeService.getSaveFilterStaet().subscribe({
      next: (res) => {
        ...
        setTimeout(() => {
          this.filterInputs.forEach((input) => {
            const col = input.nativeElement.getAttribute('data-column');
            if (col && this.filterValues[col as EmpFilterKey] !== undefined) {
              input.nativeElement.value = this.filterValues[col as EmpFilterKey];
            }
          });
        });
  ...
  onDocumentClick(event: MouseEvent) {
    ...
    const isInsideFilterBox = this.commonFilterBox?.nativeElement.contains(clickedElement);

    // Check if click is on any filter icon
    const isFilterButton = clickedElement.classList.contains('filter-btn');
