...
export class EditComponent implements OnInit {
  ...
  constructor(
    ...
    this.editForm = this.fb.group(
      {
        editEmpName: ['', [Validators.required, Validators.minLength(3), CommonValidator.empName]],
        editEmpCode: [{ value: '', disabled: true }],
        editEmpEmail: ['', [Validators.required, CommonValidator.empEmail]],
        editEmpPhone: [
          '',
  ...
  loadEmployee(): void {
    this.employeeService.getEmployeeByCode(this.empCode).subscribe({
      next: (data: Employee) => {
        this.id = data.id!;
  ...
  onSubmit() {
    ...
    const payload = this.editForm.getRawValue(); //includes disabled fields also
    ...
    // Call backend API
    this.employeeService.updateEmployee(this.id, empDTO).subscribe({
      next: (res) => {
        console.log('Backend response:', res);
        this.toast.showSuccess('Employee edited successfully');
        ...
        if (empDTO.empCode === loggedInEmpCode) {
          ...
          console.log('Header username updated to:', empDTO.empName);
        }
